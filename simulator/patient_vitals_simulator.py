...
patient_ids = [f"P{i:03d}" for i in range(1, PATIENT_COUNT + 1)] # P001, P002,...
...
def generate_vitals():
    ...
    if random.random() < ERROR_RATE:
        ...
        if error_type == "missing_field":
            ...
            field_to_remove = random.choice(list(record.keys())) # randomly choose one field
            record[field_to_remove] = None # assign None
        elif error_type == "negative_value":
            record["heart_rate"] = -1
        elif error_type == "out_of_range":
            record["spo2"] = round(random.uniform(101,150),1)  # invalid SpO2
    return record
...
while True:
    ...
    publisher.publish(topic_path, message_json.encode("utf-8")) # encoding because pubsub accepts bytes only
